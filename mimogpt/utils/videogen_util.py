...
from mimogpt.utils.convert_from_ckpt import (
    convert_ldm_unet_checkpoint,
    convert_ldm_clip_checkpoint,
    convert_ldm_vae_checkpoint,
)
from mimogpt.utils.convert_lora_safetensor_to_diffusers import convert_lora, convert_motion_lora_ckpt_to_diffusers
import cv2
...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=8):
    ...
    media_writer = cv2.VideoWriter(path, fourcc, fps, ((outputs[0]).shape[0], (outputs[0]).shape[1]), True)
    for im in outputs:
        img = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
        media_writer.write(img)
    media_writer.release()

    # imageio.mimsave(path, outputs, duration=1000/8, quality=100,loop=0)
