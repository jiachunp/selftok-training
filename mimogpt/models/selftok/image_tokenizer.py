...
class ImageTokenizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # if use_smart_react:
        #     quantizer_config["smart_re_K"] = k

        self.k_m = k_m
        self.k_s = k_s
        self.k = k
        self.t2k = t2k
        self.two_part_losses = two_part_losses
    ...
    def forward(self, x, ema=None, full_tokens=False, step=0, **kwargs):
        ...
        # if self.diti.stages is not None:
        #     t = self.batch_segment_uniform_sample(x.shape[0], self.diti.stages, self.diti.k_per_stage).cuda()
        # else:
        #     t = self.diffusion.sample_t(x.shape[0],1.0).cuda()  # lognorm

        # t = (torch.rand(x.shape[0]) ** (1/3)).cuda()
        ...
        if not self.encoder.training:
            with torch.no_grad():
                encoder_hidden_states, to_quantizer_features, ori_hidden_states, attn_mask, quan_loss, log_dict, _ = self.encoder(x=x, d=k_batch, kwargs=kwargs)
            to_quantizer_features_ema = None
        else:
            encoder_hidden_states, to_quantizer_features, ori_hidden_states, attn_mask, quan_loss, log_dict, _ = self.encoder(x=x, d=k_batch, kwargs=kwargs)
            to_quantizer_features_ema = None
            # if self.ema_enc:
            #     update_ema(self.enc_ema, self.encoder, decay=self.enc_decay)
            #     with torch.no_grad():
            #         encoder_hidden_states_ema, to_quantizer_features_ema, ori_hidden_states_ema, attn_mask_ema, quan_loss_ema, log_dict_ema = self.enc_ema(x=x, d=k_batch)

        encoder_hidden_states_d = encoder_hidden_states
        attn_mask_d = attn_mask

        # two-part loss
        if self.two_part_losses and self.encoder.training:
            bs = x.shape[0]
            original_t = t.clone()
            t[bs//2:] = 1.0
            attn_mask_d[bs//2:] = attn_mask_d[bs//2:].logical_not()
            encoder_hidden_states_d = ori_hidden_states * attn_mask_d[..., None].expand_as(ori_hidden_states)
        else:
            original_t = None
        ...
        loss_dict = self.diffusion.training_losses(
            self.model, x, t, model_kwargs, noise=noise, two_part=self.two_part_losses, original_t=original_t
        )
        # prepare logs
