...
class FSQ(Module):
    def __init__(
        ...
        if self.smart_react_K > 0:
            self.register_buffer(
                'timestep_p_over_c',
                torch.ones(num_codebooks, self.smart_react_K, self.codebook_size) / self.codebook_size
            )
    ...
    def forward(self, z):
        ...
        # embed_onehot = F.one_hot(indices.squeeze(-1), num_classes=self.codebook_size).unsqueeze(0).type(z.dtype)
        ...
        # unpacked_onehot = F.one_hot(indices.flatten(), num_classes=self.codebook_size).type(orig_dtype)
        # unpacked_onehot = unpacked_onehot.sum(dim=0)
        # distributed.all_reduce(unpacked_onehot)
        # ema_inplace(self.cluster_size.data, unpacked_onehot, decay)
