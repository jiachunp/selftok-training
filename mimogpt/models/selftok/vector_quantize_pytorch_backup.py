...
class CosineSimCodebook(nn.Module):
    def __init__(
        ...
        self.eps = eps
        self.threshold_ema_dead_code = threshold_ema_dead_code
        self.reset_cluster_size = default(reset_cluster_size, threshold_ema_dead_code * 1.2)
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        # continuous tricks
        self.register_buffer('continuous', torch.Tensor([False]))
    ...
    def forward(
        ...
        self.steps += 1
        if self.steps > 850 and self.continuous:
        ...
        log_dict = {
            ...
            "perplexity": 0,
            "delta_embed": self._codebook.delta_embed.item(),
        }
        # additional logging
        total_codes = len(embed_ind.flatten()) * dist.get_world_size()
        log_dict['num_active_codes'] = \
        ...
        if self.training and not (self.continuous):
            if self.commitment_weight > 0:
                ...
                log_dict["commitment_loss"] = commit_loss.detach().item()
