...
class ViTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, attn=Attention, **block_kwargs):
        super().__init__()
        self.norm1_vae = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.norm1_vit = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = attn(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.norm2_vae = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.norm2_vit = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.mlp_vae = Mlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0)
        self.mlp_vit = Mlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0)
...
class DualAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, query: torch.Tensor, mask: torch.Tensor=None, x_mask: torch.Tensor=None) -> torch.Tensor:
        ...
        if self.zero_init:
            ...
            k, v = kv.unbind(0)
            query_q, query_k, query_v = query_qkv.unbind(0)
            # zero_tensor = torch.zeros_like(query_k)

            # # 将需要保持的值复制到全零张量中
            # zero_tensor[:,:,:1,:] = query_k[:,:,:1,:]

            # # 现在 zero_tensor 包含了修改后的值
            # query_k = zero_tensor
            k = torch.cat([k,query_k], dim=2)
            v = torch.cat([v,query_v], dim=2)
...
class TriAttention(nn.Module):
    def __init__(
        ...
        # latent linear
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) #qkv for vae
        self.qkv_vit = nn.Linear(dim, dim * 3, bias=qkv_bias) #qkv for vit
        self.to_query_kv = nn.Linear(dim, query_dim * 2, bias=qkv_bias) \
            if (not bidirectional) or zero_init else nn.Identity()
        self.to_query_kv_vit = nn.Linear(dim, query_dim * 2, bias=qkv_bias) \
            if (not bidirectional) or zero_init else nn.Identity()
        # query linear
        self.query_linear = nn.Linear(query_dim, query_dim * 3, bias=qkv_bias)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.q_norm_vit = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm_vit = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.query_qnorm = norm_layer(self.query_head_dim) if qk_norm else nn.Identity()
        self.query_knorm = norm_layer(self.query_head_dim) if qk_norm else nn.Identity()
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_vit = nn.Linear(dim, dim)
        self.query_proj = nn.Linear(query_dim, query_dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.proj_drop_vit = nn.Dropout(proj_drop)

    def forward(self, x: torch.Tensor, x_vit: torch.Tensor, query: torch.Tensor, mask: torch.Tensor=None, x_mask: torch.Tensor=None) -> torch.Tensor:
        B, N, C = x.shape
        _, N_vit, _ = x_vit.shape
        _, query_N, query_C = query.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        qkv_vit = self.qkv_vit(x_vit).reshape(B, N_vit, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        q_vit, k_vit, v_vit = qkv_vit.unbind(0)
        q_vit, k_vit = self.q_norm_vit(q_vit), self.k_norm_vit(k_vit)
        ...
        if self.zero_init:
            ...
            query_q, query_k, query_v = query_qkv.unbind(0)
            query_q, query_k = self.query_qnorm(query_q), self.query_knorm(query_k)
            q = torch.cat((q, q_vit, query_q), dim=2)
            k = torch.cat((k, k_vit, query_k), dim=2)
            v = torch.cat((v, v_vit, query_v), dim=2)
            x_cat = F.scaled_dot_product_attention(
                q, k, v, attn_mask=mask,
                dropout_p=self.attn_drop.p if self.training else 0.,
            )
            x, x_vit, query = x_cat[:,:, :N, :], x_cat[:,:, N:N+N_vit, :], x_cat[:,:, N+N_vit:, :]
        else:
            kv = self.to_query_kv(x).reshape(B, N, 2, self.query_heads, self.query_head_dim).permute(2, 0, 3, 1, 4)
            kv_vit = self.to_query_kv_vit(x_vit).reshape(B, N_vit, 2, self.query_heads, self.query_head_dim).permute(2, 0, 3, 1, 4)
            x = F.scaled_dot_product_attention(
                q, k, v, attn_mask=x_mask,
                dropout_p=self.attn_drop.p if self.training else 0.,
            )
            x_vit = F.scaled_dot_product_attention(
                q_vit, k_vit, v_vit, attn_mask=x_mask,
                dropout_p=self.attn_drop.p if self.training else 0.,
            )
            k, v = kv.unbind(0)
            k_vit, v_vit = kv_vit.unbind(0)
            query_q, query_k, query_v = query_qkv.unbind(0)
        ...
        x = x.transpose(1, 2).reshape(B, N, C)
        x_vit = x_vit.transpose(1, 2).reshape(B, N_vit, C)
        query = query.transpose(1, 2).reshape(B, query_N, query_C)
        x = self.proj(x)
        x_vit = self.proj_vit(x_vit)
        query = self.query_proj(query)
        x = self.proj_drop(x)
        x_vit = self.proj_drop_vit(x_vit)
        query = self.proj_drop(query)
        return x, x_vit, query
...
class DualBlock(ViTBlock):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, time_adaln=False, query_dim=256, diti=None, **block_kwargs):
        ...
        super().__init__(hidden_size, num_heads, mlp_ratio, attn=TriAttention, **block_kwargs)
        q_dim = query_dim
    ...
    def forward(self, x_vae, x_vit, q, mask=None):
        if self.time_adaln:
            K = q.shape[1]
            if self.diti is not None:
                pos_embed = self.diti.get_position(torch.arange(K).to(x_vae.device))  # *7+1000
            else:
                pos_embed = torch.arange(K).to(x_vae.device)
        ...
        x_vae_attn, x_vit_attn, q_attn = self.attn(self.norm1_vae(x_vae), self.norm1_vit(x_vit), modulate(self.q_norm1(q), shift_msa, scale_msa, 0), mask=mask)
        x_vae = x_vae + x_vae_attn
        x_vae = x_vae + self.mlp_vae(self.norm2_vae(x_vae))

        x_vit = x_vit + x_vit_attn
        x_vit = x_vit + self.mlp_vit(self.norm2_vit(x_vit))
...
class DualBlockMultiRes(ViTBlock):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, time_adaln=False, query_dim=512, diti=None, low_res_hidden_dim=64, **block_kwargs):
        block_kwargs["query_dim"] = query_dim
        self.post_ln = block_kwargs.pop("post_ln", False)
        # TODO merge two classes into one
