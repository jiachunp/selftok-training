...
class JointBlock(nn.Module):
    ...
    def __init__(self, use_checkpoint, init_method, *args, **kwargs):
        ...
        # if init_method == 2:
        #     self.context_block = DismantledBlock(*args, pre_only=pre_only, qk_norm=qk_norm, post_ln=post_ln, **kwargs)
        #     self.x_block = DismantledBlock(*args, pre_only=False, qk_norm=qk_norm, init_method=init_method, **kwargs)
        # else:
        #     self.context_block = DismantledBlock(*args, pre_only=pre_only, qk_norm=qk_norm, init_method=init_method, post_ln=post_ln, **kwargs)
        #     self.x_block = DismantledBlock(*args, pre_only=False, qk_norm=qk_norm, init_method=init_method, **kwargs)
        ...
        if init_method == 2:
            ...
            self.context_block = DismantledBlock(*args, pre_only=pre_only, qk_norm=qk_norm, post_ln=post_ln, **kwargs)
            self.x_block = DismantledBlock(*args, pre_only=False, qk_norm=qk_norm, init_method=init_method, **x_kwargs)
        else:
            x_kwargs = kwargs.copy()
            x_kwargs['time_adaln'] = 't_emb'
    ...
    def forward(self, context, x, c, mask=None):
        if self.use_checkpoint:
            ...
            return self._forward(context, x, c, mask)
...
class RenderDiT(nn.Module):
    ...
    def forward(self, x0, y=None, encoder_hidden_states=None, x_mask=None, mask=None,**kwargs):
        """
        Forward pass of DiT.
        ...
        mask = mask.unsqueeze(1).unsqueeze(2).repeat(1,1,self.register_length+context.shape[1]+x.shape[1],1)

        x = self.forward_core_with_concat(x, c, context, mask=mask)
        output_x = x

        x = self.unpatchify(output_x, hw=hw)  # (N, out_channels, H, W)
        return x
