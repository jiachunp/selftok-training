...
class SelfAttention(nn.Module):
    def __init__(
        ...
        if qk_norm == "rms":
            ...
            self.ln_q = nn.RMSNorm(self.head_dim, elementwise_affine=True, eps=1.0e-6, dtype=dtype, device=device)
            self.ln_k = nn.RMSNorm(self.head_dim, elementwise_affine=True, eps=1.0e-6, dtype=dtype, device=device)
        elif qk_norm == "ln":
    ...
    def pre_attention(self, x: torch.Tensor, c: torch.Tensor):
        ...
        if not self.pre_only:
            if not self.scale_mod_only:
                ...
                if self.time_adaln == 'pos_emb' or self.time_adaln == 'pos_t_emb':
                    K = x.shape[1]
                    pos = torch.arange(K).to(x.device)
                    pos_embed = 1000 + (pos * 8)
    ...
    def forward(self, context, x, c, mask=None):
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(
                self._forward, context, x, c, mask, use_reentrant=False
            )
        else:
            return self._forward(context, x, c, mask, use_reentrant=False)
...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        pos = np.arange(context_len, dtype=np.float32)
        pos_embed = 1000 + (pos * 8)
    ...
    def forward(self, x, t, encoder_hidden_states=None, **kwargs):
        ...
        # CFG training
        context, mask, drop_ids = \
            self.drop_cond(context, mask, t)
        ...
        context_mask = context_mask.unsqueeze(1).unsqueeze(2).repeat(1, 1, context.shape[1], 1)
