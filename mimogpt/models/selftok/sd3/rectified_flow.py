...
class RectifiedFlow(torch.nn.Module):
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, recon_ratio=None, two_part=False, original_t=None):
        """
        ...
        if two_part:
            # change the second half of samples to predict xt
            bs = t.shape[0]
            T = 1.0
            # assert t[bs//2:] == T
            # assert original_t[:bs//2] == t[:bs//2]
            pred_xt = noise - v + v * rearrange(original_t, 'b -> b 1 1 1')
            gt_xt = self.q_sample(x_start, original_t, noise=noise)
            model_output[bs//2:] = pred_xt[bs//2:]
            target[bs//2:] = gt_xt[bs//2:]
        ...
        for i, step in enumerate(self.scheduled_t):
            ...
            with torch.no_grad():
                if cond_vary:
                    ...
                    # t_lowres = self.hr2lr_align_t(t, self.alpha_hr2lr_align_t)  # => 256 noise t
                    # # t_lowres = self.shift_t_reverse(t_lowres, self.shift)  # 256 shift is 1.0
                    # k_lowres = diti.to_indices(t_tmp_lowres)
