...
class Encoder(nn.Module):
    def __init__(
        ...
        pre_norm=False, post_norm=True, encoder_out_dim=None, gradient_checkpointing=False, 
        pos_embed_max_size=None, quantizer_config=None, attn_mask=False, single_token=False, **kwargs
    ):
        super().__init__()
        self.K = K
        ...
        self.vit_linear = nn.Linear(2048, hidden_size)
        ...
        self.vit_linear_post = nn.Linear(hidden_size, 2048)
    ...
    def get_perplexity_list(self, log_dict, chunks=50):
        if 'perplexity_list' in log_dict:
            ...
            return perplexity_list.tolist(), None
    ...
    def forward(self, x_vae=None, x_vit=None, hidden_states=None, d=None, kwargs=None):
        """
        Forward pass of feature encoder.
        x_vae: (N, C, H, W) tensor of spatial inputs (vae latent representations of images)
        d: N, the depth for each sample
        """
        if self.pos_embed_max_size is not None:
            hw = x_vae.shape[-2:]
            x_vae = self.x_embedder(x_vae)
            x_vae = x_vae + self.cropped_pos_embed(hw) #torch.Size([4, 64, 512])
        else:
            x_vae = self.x_embedder(x_vae) + self.pos_embed

        x_vit = self.vit_linear(x_vit) 


        if hidden_states is None:
            outs = self.get_encoder_outs(x_vae, x_vit, kwargs=kwargs)
            if self.pre_norm:
            ...
            # print("to_quantizer_features", to_quantizer_features, to_quantizer_features.shape)
        ...
        enc_mask = self.get_encoder_mask(x_vae, d)
        attn_mask = enc_mask
        mask_v = enc_mask[..., None].expand_as(outs_q)
        encoder_hidden_states = outs_q * mask_v
        # outs_q = self.vit_linear_post(outs_q)
        to_quantizer_features = self.vit_linear_post(to_quantizer_features)
...
class QformerEncoder(Encoder):
    def __init__(
        ...
        gradient_checkpointing=False, pos_embed_max_size=None, xavier_init=False, diti=None, 
        quantizer_config=None, attn_mask=False, single_token=False, **kwargs
    ):
        super().__init__(
            ...
            gradient_checkpointing=gradient_checkpointing, pos_embed_max_size=pos_embed_max_size, 
            quantizer_config=quantizer_config, attn_mask=attn_mask, single_token=single_token, **kwargs
        )
        qformer_depth = depth
    ...
    def get_encoder_outs(self, x_vae, x_vit, kwargs=None):
        query_tokens = self.query_tokens.expand(x_vae.shape[0], -1, -1)
        if self.mode == 'qformer':
            query_tokens = self.qformer(x_vae, query_tokens) # [B, L, C]
        elif self.mode == 'concat':
            for i, block in enumerate(self.blocks):
                if self.gradient_checkpointing:
                    x_vae, x_vit, query_tokens = checkpoint(ckpt_wrapper(block), x_vae, x_vit, query_tokens, use_reentrant=False)
                else:
                    x_vae, x_vit, query_tokens = block(x_vae, x_vit, query_tokens)
        elif self.mode == 'dual':
            # attn mask
            if self.attn_mask:
                mask = mask = torch.ones(self.K, self.K).tril().bool().cuda()
                x_mask = torch.ones((self.K, x_vae.shape[1])).cuda()
                mask = torch.cat((x_mask, mask), dim=1).bool()
                mask = mask.unsqueeze(0).unsqueeze(1).repeat(x_vae.shape[0],1,1,1)
            else:
                mask = None
            ...
            for i, block in enumerate(self.blocks):
                if self.gradient_checkpointing:
                    x_vae, x_vit, query_tokens = checkpoint(ckpt_wrapper(block), x_vae, x_vit, query_tokens, mask, use_reentrant=False)
                else:
                    x_vae, x_vit, query_tokens = block(x_vae, x_vit, query_tokens, mask=mask)
                if to_print and (i < 4 or i > 12):
                    print(f"Encoder Layer {i}: max(x)={x_vae.abs().max(dim=-1)[0].mean().item()}, mean(x)={x_vae.abs().mean(dim=-1).mean().item()}; max(q)={query_tokens.abs().max(dim=-1)[0].mean().item()}, mean(q)={query_tokens.abs().mean(dim=-1).mean().item()}")
        else:
...
#         hidden_size=256, depth=None, num_heads=4, mlp_ratio=4.0,
#         pre_norm=False, post_norm=True, qformer_mode='qformer',
#         gradient_checkpointing=False, pos_embed_max_size=None, 
