...
parser.add_argument("--model_path", type=str, default="/home/jovyan/weijiawu/bagel_selftok_qwenvl/ckpt_path_imagenet1k/0002000")
...
# # merge dicts
# model_state_dict = {**shard1, **shard2}
...
#     model = Bagel(vl_model, config).eval()


# tokenizer = Qwen2Tokenizer.from_pretrained(qwen_vl_path)
# tokenizer, new_token_ids, _ = add_special_tokens(tokenizer)
...
def multi_round_edit():
    image = load_example_image('test_images/women_5.jpg')
    prompt = "Put a hat on the person in the picture."
    ...
    def process_edit_image(image, prompt, show_thinking, cfg_text_scale, 
            ...
            return edited_image, thinking

    edited_image, thinking = process_edit_image(image, prompt, show_thinking, cfg_text_scale, 
                              cfg_img_scale, cfg_interval, 
                              timestep_shift, num_timesteps, cfg_renorm_min, 
                              cfg_renorm_type, max_think_token_n, do_sample, 
                              text_temperature, seed)
    print(thinking)
    edited_image.save("test_images/women_6.jpg")


def multi_round_under():
    image = load_example_image('test_images/women.jpg')
    ...
    result = process_understanding(image, prompt, show_thinking, do_sample, 
                              text_temperature, max_new_tokens)
    print("-----------------------------")
