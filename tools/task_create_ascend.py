...
def get_basic_args(job_cfg, args):
    ...
    return job_name, app_name, bucket, region, priority, code_root, prefix
...
def task_create(args, unknown):
    ...
    job_name, app_name, bucket, region, priority, code_root, prefix = get_basic_args(job_cfg, args)
    ...
    if args.auto_upload_code:
        cmd = (
            ...
            f"python tools/code_upload_win.py "
            f"--app {app_name} "
            f"--code_root {code_root} "
            f"--bucket {bucket} "
            f"--region {region} "
            f"--modelarts_cfg_path {args.modelarts_cfg_path}"
        )
    ...
    task_cfg = {
        ...
        "running_script": "train_cloud_ascend.py",
        "running_args": {
            "args_yml_fn": None,
            "bucket": bucket,
            "user_id": user_id,
