...
def _join_rocm_home(*paths) -> str:
    ...
    """
    if ROCM_HOME is None:
        raise EnvironmentError("ROCM_HOME environment variable is not set. " "Please set it to your ROCm install root.")
    elif IS_WINDOWS:
        raise EnvironmentError("Building PyTorch extensions using " "ROCm and Windows is not supported.")
    ...
    def build_extensions(self) -> None:
        ...
        def unix_cuda_flags(cflags):
            ...
            if _ccbin is not None and not any(
                [flag.startswith("-ccbin") or flag.startswith("--compiler-bindir") for flag in cflags]
            ):
...
def CUDAExtension(name, sources, *args, **kwargs):
    ...
    if dlink:
        ...
        if (torch.version.cuda is not None) and packaging.version.parse(torch.version.cuda) >= packaging.version.parse(
            "11.2"
        ):
    ...
    if functions is not None:
        ...
        for function_name, docstring in functions.items():
            if with_pytorch_error_handling:
                module_def.append(
                    'm.def("{0}", torch::wrap_pybind_function({0}), "{1}");'.format(function_name, docstring)
                )
            else:
...
def _join_cuda_home(*paths) -> str:
    ...
    """
    if CUDA_HOME is None:
        raise EnvironmentError("CUDA_HOME environment variable is not set. " "Please set it to your CUDA install root.")
