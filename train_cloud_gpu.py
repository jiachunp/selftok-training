...
if __name__ == "__main__":
    ...
    # -----------------------environment-----------------------
    parser.add_argument("--requirements", type=str, default="requirements_gpu.txt", help="requirements file")
    ...
    from cloud_copy import common_cloud_copy, universal_cloud_copy, prepare_imagenet, prepare_openimage, prepare_sd3_zipdata
    ...
    if os.path.exists("/cache/data/imagenet/features/"):
        prepare_imagenet()
    if os.path.exists("/cache/sd1.5-features/openimage256_features/"):
        prepare_openimage()
    if yml.dataloader.get('prepare_zip',False):
        prepare_sd3_zipdata(yml.dataloader)
    ...
    if hasattr(yml.common, "use_deepspeed") and yml.common.use_deepspeed:
        ...
        for fake_rank in range(ngpus_per_node):
            ...
            cmd = "cd {}; conda activate tokenizer; export LD_LIBRARY_PATH=/home/ma-user/modelarts/lib:$LD_LIBRARY_PATH;".format(project_path)
        ...
        for fake_rank in range(ngpus_per_node):
            ...
            taskset = "taskset -c {}-{}".format(core_per_proc * fake_rank, core_per_proc * (fake_rank + 1) - 1)
            # export LD_LIBRARY_PATH solve the error "OSError: libaccess_sdk.so" when using memarts
            cmd = "cd {};export LD_LIBRARY_PATH=/home/ma-user/modelarts/lib:$LD_LIBRARY_PATH; export CRYPTOGRAPHY_OPENSSL_NO_LEGACY=1;".format(project_path)
            cmd += "OMP_NUM_THREADS=4 {} /home/ma-user/anaconda3/envs/tokenizer/bin/python train_net.py".format(taskset)
