...
class PackedDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        # 只取T2I的dataloader
        self.dataset_iters = [iter(dataset) for dataset in grouped_datasets[:1]]
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            for item in dataset_names:
                ...
                if "token_dir" in meta_info.keys():
                    if "token_dir" not in dataset_args:
                        dataset_args['token_dir'] = []

                    dataset_args['token_dir'].append(meta_info['token_dir'])
    ...
    def set_sequence_status(self):
        sequence_status = dict(
            ...
            packed_selftok_ids          = list(),
            packed_selftok_indexes      = list(),
            packed_selftok_label_ids    = list(),
            packed_selftok_position_ids = list(),
            selftok_ce_loss_indexes     = list(),
            selftok_ce_loss_weights     = list(),
        )
    ...
    def to_tensor(self, sequence_status):
        ...
        # if the model is required to perform visual generation
        if len(sequence_status['packed_selftok_ids']) > 0:
            data['packed_selftok_ids'] = torch.tensor(sequence_status['packed_selftok_ids'])
            data['packed_selftok_indexes'] = torch.tensor(sequence_status['packed_selftok_indexes'])
            data['packed_selftok_label_ids'] = torch.tensor(sequence_status['packed_selftok_label_ids'])
            data['packed_selftok_position_ids'] = torch.tensor(sequence_status['packed_selftok_position_ids'])
            data['selftok_ce_loss_indexes'] = torch.tensor(sequence_status['selftok_ce_loss_indexes'])
            data['selftok_ce_loss_weights'] = torch.tensor(sequence_status['selftok_ce_loss_weights'])
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                shifted_text_ids = [self.bos_token_id] + text_ids    # teacher forcing
                ...
                sequence_status['packed_position_ids'][0].extend(range(curr_rope_id, curr_rope_id + curr_split_len))
                sequence_status['packed_position_ids'][1].extend(range(curr_rope_id, curr_rope_id + curr_split_len))
                sequence_status['packed_position_ids'][2].extend(range(curr_rope_id, curr_rope_id + curr_split_len))
            ...
            elif item['type'] == 'selftok_image':
                image_tensor = image_tensor_list.pop(0)[0][::-1]
                num_tokens = sample['num_tokens']
                # 可选：配一个 selftok_cond_dropout_prob（用 vae_cond_dropout_prob 也行）
                if item['enable_cfg'] == 1 and random.random() < self.data_config.selftok_cond_dropout_prob:
                    curr_rope_id += 1
                    continue
                ...
                # preprocess image
                sequence_status['packed_selftok_ids'].extend(image_tensor)
                sequence_status['packed_selftok_indexes'].extend(range(curr, curr + len(image_tensor)))
                sequence_status['packed_selftok_position_ids'].extend(torch.arange(0, len(image_tensor)))
                
                # CE LOSS:
                if item['loss'] == 1:
                    sequence_status['selftok_ce_loss_indexes'].extend(range(curr-1, curr + len(image_tensor) - 1)) #zfd: teacher forcing
                    # sequence_status['selftok_ce_loss_weights'].extend(
                    #     [len2weight(len(image_tensor))] * len(image_tensor)
                    # )
                    #zfd
                    sequence_status['selftok_ce_loss_weights'].extend(
                        [1.0] * len(image_tensor)
                    )
                    sequence_status['packed_selftok_label_ids'].extend(image_tensor)
                    # sequence_status['packed_selftok_label_ids'].extend([self.end_of_image]) #zfd
                curr += len(image_tensor)
                curr_split_len += len(image_tensor)
                ...
                # update sequence status
                attn_modes.append("causal")
                sequence_status['packed_position_ids'][0].extend([curr_rope_id] * (len(image_tensor) + 2))
                sequence_status['packed_position_ids'][1].extend([curr_rope_id] * (len(image_tensor) + 2))
                sequence_status['packed_position_ids'][2].extend([curr_rope_id] * (len(image_tensor) + 2))
                curr_rope_id += 1
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        if "packed_selftok_indexes" in data.keys():
            self.packed_selftok_indexes = data["packed_selftok_indexes"]
            self.packed_selftok_ids = data["packed_selftok_ids"]
            self.packed_selftok_label_ids = data["packed_selftok_label_ids"]
            self.packed_selftok_position_ids = data["packed_selftok_position_ids"]
            self.selftok_ce_loss_indexes = data["selftok_ce_loss_indexes"]
            self.selftok_ce_loss_weights = data["selftok_ce_loss_weights"]
    ...
    def pin_memory(self):
        ...
        if hasattr(self, 'packed_timesteps'):
            self.packed_selftok_indexes = self.packed_selftok_indexes.pin_memory()
            self.packed_selftok_ids = self.packed_selftok_ids.pin_memory()
            self.packed_selftok_label_ids = self.packed_selftok_label_ids.pin_memory()
            self.packed_selftok_position_ids = self.packed_selftok_position_ids.pin_memory()
            self.selftok_ce_loss_indexes = self.selftok_ce_loss_indexes.pin_memory()
            self.selftok_ce_loss_weights = self.selftok_ce_loss_weights.pin_memory()
    ...
    def cuda(self, device):
        ...
        if hasattr(self, 'packed_selftok_indexes'):
            self.packed_selftok_indexes = self.packed_selftok_indexes.to(device)
            self.packed_selftok_ids = self.packed_selftok_ids.to(device)
            self.packed_selftok_label_ids = self.packed_selftok_label_ids.to(device)
            self.packed_selftok_position_ids = self.packed_selftok_position_ids.to(device)
            self.selftok_ce_loss_indexes = self.selftok_ce_loss_indexes.to(device)
            self.selftok_ce_loss_weights = self.selftok_ce_loss_weights.to(device)
    ...
    def to_dict(self):
        ...
        if hasattr(self, 'packed_selftok_indexes'):
            data['packed_selftok_indexes'] = self.packed_selftok_indexes
            data['packed_selftok_ids'] = self.packed_selftok_ids
            data['packed_selftok_label_ids'] = self.packed_selftok_label_ids
            data['packed_selftok_position_ids'] = self.packed_selftok_position_ids
            data['selftok_ce_loss_indexes'] = self.selftok_ce_loss_indexes
            data['selftok_ce_loss_weights'] = self.selftok_ce_loss_weights
