...
from data.dataset_base_selftok import DataConfig, PackedDataset, collate_wrapper
...
def logit_normal_pdf(x, mu=0.0, sigma=1.0):
    # x in (0,1), mu in R, sigma > 0
    z = (logit(x) - mu) / sigma
    # 1 / (x(1-x) * sigma * sqrt(2*pi)) * exp(-0.5 * z^2)
    return np.exp(-0.5 * z**2) / (x * (1.0 - x) * sigma * np.sqrt(2.0 * np.pi))

@dataclass
class ModelArguments:
    model_path: str = field(
        default="hf/Selftok-7B-MoT",
        metadata={"help": "Path of the pretrained model."}
    )
    vl_path: str = field(
        default="hf/Qwen2.5-0.5B-Instruct/",
        metadata={"help": "Path or HuggingFace repo ID of the pretrained Qwen2-style language model."}
    )
    vl_qk_norm: bool = field(
        default=False,
    ...
    layer_module: str = field(
        default="Qwen2_5_VLMoTDecoderLayer",
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        # training_args.allow_val_change=True
    ...
    if training_args.finetune_from_hf:
        vl_config = Qwen2_5_VLConfig.from_json_file(os.path.join(model_args.model_path, "config.json"))
    else:
        vl_config = Qwen2_5_VLConfig.from_pretrained(model_args.vl_path)
    vl_config.text_config.layer_module = model_args.layer_module
    vl_config.text_config.qk_norm = model_args.vl_qk_norm
    vl_config.text_config.tie_word_embeddings = model_args.tie_word_embeddings
    vl_config.text_config.freeze_und = training_args.freeze_und
    if training_args.finetune_from_hf:
        vl_model = Qwen2_5_VLForCausalLM(vl_config)
    else:
        vl_model = Qwen2_5_VLForCausalLM.from_pretrained(model_args.vl_path, config=vl_config)
    # print(vl_model.state_dict().keys())
    ...
    tokenizer = Qwen2Tokenizer.from_pretrained(model_args.model_path if training_args.finetune_from_hf else model_args.vl_path)
    tokenizer, new_token_ids, num_new_tokens = add_special_tokens(tokenizer)
    if num_new_tokens > 0:
        model.vl_model.model.language_model.resize_token_embeddings(len(tokenizer))
        model.config.vocab_size = len(tokenizer)
        model.vl_model.model.language_model.config.vocab_size = len(tokenizer)
    ...
    if training_args.freeze_llm:
        model.vl_model.model.language_model.eval()
        for param in model.vl_model.model.language_model.parameters():
            param.requires_grad = False
    if training_args.freeze_vit and training_args.visual_und:
        model.vl_model.model.visual.eval()
        for param in model.vl_model.model.visual.parameters():
            param.requires_grad = False
    ...
    if training_args.visual_gen:
        ...
        dataset_config.selftok_cond_dropout_prob = model_args.selftok_cond_dropout_prob
    ...
    # if training_args.visual_gen: #zfd
    #     vae_model.to(device).eval() #zfd
    fsdp_model.train()
    ema_model.eval()

    eps = 1e-5
    x = np.linspace(eps, 1.0 - eps, 3072)
    selftok_ce_weight = torch.from_numpy(logit_normal_pdf(x)[:1536])
    # train loop
    start_time = time()
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        selftok_ce_loss_weights = data.pop('selftok_ce_loss_weights', None)
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            # print(data.keys())
            # dict_keys(['sequence_length', 'sample_lens', 'packed_text_ids', 'packed_text_indexes', 'packed_position_ids', 'nested_attention_masks', 'padded_images', 'patchified_vae_latent_shapes', 'packed_latent_position_ids', 'packed_vae_token_indexes', 'packed_vit_tokens', 'packed_vit_position_ids', 'packed_vit_token_indexes', 'vit_token_seqlens', 'packed_timesteps', 'mse_loss_indexes', 'packed_label_ids', 'ce_loss_indexes'])
            # if training_args.visual_gen:
            #     with torch.no_grad():
        ...
        if training_args.visual_gen:
            selftok_ce = loss_dict["selftok_ce"]
            total_selftok_ce_tokens = torch.tensor(len(data['selftok_ce_loss_indexes']), device=device)
            total_selftok_img = total_selftok_ce_tokens // 1536 
            # expand weight to match token count
            num = total_selftok_img.item()
            expanded_weight = selftok_ce_weight.repeat(num).to(selftok_ce.device)
            # apply weighting
            weighted_ce = (selftok_ce * expanded_weight).sum()
            dist.all_reduce(total_selftok_ce_tokens, op=dist.ReduceOp.SUM)
            weighted_ce = weighted_ce.sum() * dist.get_world_size() / total_selftok_ce_tokens #zfd
            # selftok_ce = selftok_ce.mean() #zfd
            loss_dict["selftok_ce"] = weighted_ce.detach()
            # loss = loss + selftok_ce * training_args.selftok_ce_weight
            loss = loss + weighted_ce
        else:
            assert not training_args.visual_gen
            loss_dict["selftok_ce"] = torch.tensor(0, device=device)
            total_selftok_ce_tokens = torch.tensor(0, device=device)
        ...
        if curr_step % training_args.log_every == 0:
            ...
            wandb_log['total_selftok_ce_tokens'] = total_selftok_ce_tokens.item()
